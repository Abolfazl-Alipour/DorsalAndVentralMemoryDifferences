...
from loadPretrainedclassifierAndIncorporateitintoESNForRotatingObjLeakyRatechanges import ESNRotatingObjectLrnLeakyRate
from loadPretrainedclassifierAndIncorporateitintoESNForFixedWidthLeakyRatechanges import ESNFixedWidthLrnLeakyRate
...
# tmp = Parallel(n_jobs=num_cores)(delayed(ESNRotatingObjectLrnLeakyRate)(leaky_rate,nIterations,randomSeedList[j]) for leaky_rate in intialLeakyRateVec)
...
# Saving the results:
with open('/N/u/aalipour/Karst/2ndYearProject/2020Wave/code/ReviewersAsked/code/ESNObjLeakyRateLrnslr005EqlClsExp.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([rotatObjAccuracyMat, rotatObjleakyRateMat], f)
    # f.close()
# Getting back the objects:
with open('/N/u/aalipour/Karst/2ndYearProject/2020Wave/code/ReviewersAsked/code/ESNObjLeakyRateLrns.pkl','rb') as f:  # Python 3: open(..., 'rb')
...
# Saving the results:
with open('/N/u/aalipour/Karst/2ndYearProject/2020Wave/code/ReviewersAsked/code/ESNWidthLeakyRateLrnslr005.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([fixedWidthAccuracyMat, fixedWidthleakyRateMat], f)
    # f.close()
# Getting back the objects:
with open('/N/u/aalipour/Karst/2ndYearProject/2020Wave/code/ReviewersAsked/code/ESNWidthLeakyRateLrnslr005.pkl','rb') as f:  # Python 3: open(..., 'rb')
...
#################################### FOR HIGHER LEARNING RATEW (0.005)
...
#################################### FOR lower LEARNING RATEW (0.0025)
...
ax.errorbar(g50width[0][-1], 1-g50width[1][-1], yerr=widthSEM[1,-1],xerr=widthAccSEM[1,-1]*100,capsize=capSizeVal,label='Orientation classification Memory Coefficient')
ax.set_xlim(75, 100)
# ax.set_ylim(0.1, 0.8)
...
ax.errorbar(g75width[0][-1], 1-g75width[1][-1], yerr=widthSEM[2,-1],xerr=widthAccSEM[2,-1]*100,capsize=capSizeVal,label='Orientation classification Memory Coefficient')
ax.set_xlim(75, 100)
# ax.xticks([80,85,90,100])
# ax.set_ylim(0.1, 0.8)
...
sns.catplot(x='obj',y='ori',data=[['obj',1-obj[:,0,17]],['ori',1-width[:,0,17]]],kind="point")

sns.swarmplot(data = df, x = 'obj', y = 'ori',palette=myPal)
plt.show()


sns.catplot(x="obj", y="ori", 
