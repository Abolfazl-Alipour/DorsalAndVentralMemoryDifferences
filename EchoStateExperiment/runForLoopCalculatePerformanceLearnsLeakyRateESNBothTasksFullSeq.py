# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 23:24:46 2019
...
from loadPretrainedclassifierAndIncorporateitintoESNForRotatingObjLeakyRatechangesFullSeq import ESNRotatingObjectLrnLeakyRate
from loadPretrainedclassifierAndIncorporateitintoESNForFixedWidthLeakyRatechangesFullSeq import ESNFixedWidthLrnLeakyRate
...
for i in range(obj.shape[1]):
    for j in range(obj.shape[2]):
        objSTD[i,j]=np.std(obj[:,i,j])

objMean=np.empty([3,4])
for i in range(obj.shape[1]):
    for j in range(obj.shape[2]):
...
for i in range(width.shape[1]):
    for j in range(width.shape[2]):
     widthSTD[i,j]=(np.std(width[:,i,j]))

widthMean=np.empty([3,4])
widthMean[:]=np.nan
...
for i in range(widthAcc.shape[1]):
    for j in range(widthAcc.shape[2]):
     widthAccSTD[i,j]=(np.std(widthAcc[:,i,j]))

widthAccMean=np.empty([3,4])
widthAccMean[:]=np.nan
...
colorsObj = ("cornflowerblue","royalblue","b","mediumblue")
colorsWidth=("lightcoral","indianred","firebrick","maroon")
...
ax.scatter(g25obj[0], g25obj[1], alpha=0.8, c=colorsObj, edgecolors='none', s=30,label='Object classification Leaky Rate')
ax.scatter(g25width[0], g25width[1], alpha=0.8, c=colorsWidth, edgecolors='none', s=30,label='Width classification Leaky Rate')
xy_line = (0.25, 0.25)
ax.plot(xy_line, 'k--', label='Initial Leaky Rate')
ax.set_xlabel('Network Accuracy',size=24)
ax.set_ylabel('Leaky Rate',size=24)
...
ax.tick_params(labelsize=16) 
ax.scatter(g50obj[0], g50obj[1], alpha=0.8, c=colorsObj, edgecolors='none', s=30,label='Object classification Leaky Rate')
ax.scatter(g50width[0], g50width[1], alpha=0.8, c=colorsWidth, edgecolors='none', s=30,label='Width classification Leaky Rate')
ax.set_xlim(0.2, 1)
ax.set_ylim(0.15, 0.8)
xy_line = (0.5, 0.5)
ax.plot(xy_line, 'k--', label='Initial Leaky Rate')
...
ax.tick_params(labelsize=16) 
ax.scatter(g75obj[0], g75obj[1], alpha=0.8, c=colorsObj, edgecolors='none', s=30,label='Object classification Leaky Rate')    
ax.scatter(g75width[0], g75width[1], alpha=0.8, c=colorsWidth, edgecolors='none', s=30,label='Width classification Leaky Rate')
ax.set_xlim(0.2, 1)
ax.set_ylim(0.15, 0.8)
xy_line = (0.75, 0.75)
ax.plot(xy_line, 'k--', label='Initial Leaky Rate')
