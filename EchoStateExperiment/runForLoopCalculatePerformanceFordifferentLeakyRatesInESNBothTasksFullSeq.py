...
from loadPretrainedOriclassifierAndIncorporateitintoESNForRotatingObjFullSeq import ESNRotatingObject
#from loadPretrainedOriclassifierAndIncorporateitintoESNForFixedOriObj import *
from loadPretrainedOriclassifierAndIncorporateitintoESNForFixedWidthFullSeq import ESNFixedWidth
...
for kk in tqdm(range(len(nHiddSizeVec))):
    for j in range(len(randomSeedList)):
        ...
        for leaky_rate in leaky_rateVec:
            resultsRotObj[j,i,kk]=ESNRotatingObject(leaky_rate,n_iterations,randomSeedList[j],n_hidden =nHiddSizeVec[kk],use_cuda=True )
            i=i+1
