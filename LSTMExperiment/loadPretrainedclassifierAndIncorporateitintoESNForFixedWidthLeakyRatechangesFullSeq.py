# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 23:29:55 2019
...
from tomDatasetFrameSeriesAllClassesFixedwidthFullSeq import tomImageFolderFrameSeriesAllClasses
...
def ESNFixedWidthLrnLeakyRate(leaky_rate=0.5, n_iterations=1,seed=1234567890):
    # Parameters
    spectral_radius = 0.9
    ...
    class column(nn.Module):
        ...
        """
        def __init__(self,preTrainedModel,leaky_rate,spectral_radius = 0.9,n_hidden = 200,numOfClasses=16,lastLayerSize=60):
    ...
    train_set = tomImageFolderFrameSeriesAllClasses(root=os.path.join('E:\\', 'Abolfazl' , '2ndYearproject' , 'datasets','fixedOrirentationWidth' ),transform=transform)
    
    test_set = tomImageFolderFrameSeriesAllClasses(root=os.path.join('E:\\', 'Abolfazl' , '2ndYearproject' ,  'datasets','fixedOrirentationWidth' ),transform=transform)
    ...
    for epoch in range(n_iterations):
        ...
        leakyRateList.append(c1.echo.leaky_rate.detach().numpy().astype(np.float))
        # Test reservoir
        ...
        accuracy.append((totalNumberOfOutputs-numberOfMistakes)/totalNumberOfOutputs)
        # end for
